...
except ImportError:  # pragma: no cover - allows text-only mode without PyG
    RGCNConv = None


def weighted_belief_fusion(alpha_list, weight_list, eps=1e-8, num_classes=2):
    """Weighted evidence fusion for multiple Dirichlet sources."""
...
class TextFinalSemanticHead(nn.Module):
    ...
    def forward(self, final_feat, cfg_text=None):
        ...
        return {
            ...
            "u_text": u_text,
            "alpha_from_logits": True,
        }


class TextClasswiseEvidenceHead(nn.Module):
    """
    Deterministic class-wise evidence head.
    Keeps a standard semantic logits path, but lets uncertainty be parameterized
    by class-wise evidence rather than a single scalar concentration.
    """
    def __init__(self, in_dim=4096, hid_dim=256, num_classes=2, dropout=0.1):
        super().__init__()
        self.num_classes = num_classes
        ...
        self.evidence_head = nn.Linear(hid_dim, num_classes)
    ...
    def forward(self, final_feat, cfg_text=None):
        ...
        h_unc = self.uncertainty_adapter(final_feat)
        evidence = F.softplus(self.evidence_head(h_unc))
        alpha_t = evidence + 1.0
        concentration = evidence.sum(dim=1, keepdim=True)
        u_text = self.num_classes / alpha_t.sum(dim=1, keepdim=True)
        temperature_text = torch.ones_like(u_text)
...
class TextVIBEDLHead(nn.Module):
    ...
    def forward(self, final_feat, cfg_text=None):
        cfg_text = cfg_text or {}
        scale_max = float(cfg_text.get("vib_scale_max", 3.0))

        h = self.encoder(final_feat)
        mu = self.fc_mu(h)
        logvar = self.fc_logvar(h)
        logvar = torch.clamp(logvar, min=self.logvar_min.item(), max=self.logvar_max.item())
        z = self.reparameterize(mu, logvar)

        logits = self.classifier(z)
        evidence = F.softplus(self.evidence_layer(z))
        scale = F.softplus(self.evidence_scale)
        if scale_max is not None:
            scale = torch.clamp(scale, max=scale_max)
        evidence = evidence * scale

        alpha = evidence + 1.0
        S = alpha.sum(dim=1, keepdim=True)
        u_text = self.num_classes / S

        vib_kl = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1, keepdim=True)

        temperature_text = torch.ones_like(u_text)
        concentration_text = torch.ones_like(u_text) * scale
...
class GraphEvidenceEncoder(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, num_classes=2,
        ...
        if RGCNConv is None:
            raise ImportError(
                "torch_geometric is required for GraphEvidenceEncoder. "
                "Install torch-geometric for full LM+Graph training, or use --text mode."
            )
