...
| `trainer_distillation.py` | Distillation pipeline trainers and legacy graph seed execution | `dataloader`, `model_building`, `runtime_env`, `trainer_distillation_metrics`, `trainer_indexing` | Keep trainer classes, stage entrypoint, and dynamic legacy graph-seed behavior here; shared gain/cost metrics and pseudo-label index guards now have separate owners. |
...
| `trainer_indexing.py` | Shared tensor-index normalization and pseudo-label training-index guard helpers | `torch` | Keep split-safe indexing helpers here; no trainer classes, artifact writes, CLI policy, or metric reporting. |
...
Coverage note: this index was refreshed from an AST scan of 48 non-generated
...
| `trainer_distillation.py` | Distillation trainers, stage entrypoint, and legacy graph seed execution | Imports `dataloader`, `model_building`, `runtime_env`, `trainer_distillation_metrics`, and `trainer_indexing`; re-exports private helper names for compatibility. |
...
| `trainer_indexing.py` | Shared `_as_long_cpu_tensor` and split-safe `_safe_pseudo_label_training_index` helpers | Imports only `torch`; consumed by distillation and legacy trainer paths. |
...
5. Continue splitting `trainer_distillation.py`: gain/cost metric helpers now
   live in `trainer_distillation_metrics.py`, and split-safe pseudo-label
   indexing helpers now live in `trainer_indexing.py`; next isolate trainer
   classes from `run_legacy_graph_seed` only after the training/runner boundary
   is mapped.
